...
export function EditStayDialog({ stay, isOpen, onClose }: EditStayDialogProps) {
  ...
  useEffect(() => {
    if (stay) {
      setFormData({
        place: stay.place,
        roomPrice: stay.roomPrice.toString(),
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update stay accommodation details
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? "Updating..." : "Update Stay"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
